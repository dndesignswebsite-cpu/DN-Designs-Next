...
export const getUserById = async (userId) => {
  ...
  if (!user) {
    throwError(`User not found with id of ${userId}`, 404, { function: 'getUserById', userId });
  }
  return user;
};

export const updateUser = async (userId, updateData, currentUser, avatarBuffer = null) => {
  ...
  if (currentUser.id !== userId && currentUser.role !== 'admin') {
    throwError('Not authorized to update this user', 403, { function: 'updateUser', userId });
  }
    ...
    try {
      ...
      if (existingUser.avatar && existingUser.avatar.includes('cloudinary')) {
        try {
          ...
          logError(error, { function: 'updateUser', operation: 'deleteOldAvatar' });
        }
      }

      updateData.avatar = result.secure_url;
    } catch (error) {
      throwError(error, 500, { function: 'updateUser', operation: 'uploadAvatar' });
    }
  }
  ...
  if (updateData.email && updateData.email !== existingUser.email) {
    ...
    if (emailExists) {
      throwError('Email already exists', 400, { function: 'updateUser', email: updateData.email });
    }
  }
...
export const deleteUser = async (userId, currentUser) => {
  ...
  const user = await userRepository.findById(userId);
  if (!user) {
    throwError(`User not found with id of ${userId}`, 404, { function: 'deleteUser', userId });
  }
...
export const updateProfile = async (userId, updateData, avatarBuffer = null) => {
  const user = await userRepository.findById(userId);
  if (!user) {
    ...
    try {
      ...
      if (user.avatar && user.avatar.includes('cloudinary')) {
        try {
          ...
          logError(error, { function: 'updateProfile', operation: 'deleteOldAvatar' });
        }
      }

      updateData.avatar = result.secure_url;
    } catch (error) {
      throwError(error, 500, { function: 'updateProfile', operation: 'uploadAvatar' });
    }
  }
  ...
  if (updateData.email && updateData.email !== user.email) {
    const emailExists = await userRepository.existsByEmail(updateData.email);
    if (emailExists) {
      throwError('Email already exists', 400, { function: 'updateProfile', email: updateData.email });
    }
  }
