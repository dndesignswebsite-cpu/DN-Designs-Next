...
async function getCategorySEO(slug) {
  try {
    const res = await fetch(
      `https://dndesigns.co.in/api/categories/${slug}`,
      {
        cache: "no-store",
      }
    );
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      ...
      title:
        seo.openGraph?.title || seo.metaTitle || seo.name,
...
export default async function Page({ params }) {
  ...
  return <CategoryPageComp slug={slug} />;
}
